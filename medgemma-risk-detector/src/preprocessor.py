...
Image preprocessing pipeline for the MedGemma risk detector.
Applies resize, custom LungMaskTransform (segmentation), and normalization.
...
def preprocess_classifier(image_path: str, config: dict) -> np.ndarray:
    """
    Preprocess chest X-ray image for the EfficientNet-B4 binary classifier.
    ...
    Returns:
        np.ndarray: Preprocessed image tensor with shape (1, 3, 380, 380)
...
def preprocess_vlm(image_path: str, config: dict) -> np.ndarray:
    """
    Preprocess image for MedGemma VLM (SigLIP Vision Encoder).
    ...
    Returns:
        np.ndarray: Preprocessed pixel values matching SigLIP format.
    """
    target_size = config["image_settings"]["medgemma_resize"]

    # 1. Load image
    img = Image.open(image_path).convert("RGB")

    # 2. Resize to VLM expected dimension
    img_resized = img.resize((target_size, target_size), Image.Resampling.BILINEAR)

    # 3. Normalize to [-1, 1] (SigLIP default)
    img_np = np.array(img_resized).astype(np.float32) / 255.0
    img_normalized = (img_np - 0.5) / 0.5

    # 4. Transpose to CHW and add batch dimension -> (1, 3, target_size, target_size)
    img_chw = np.transpose(img_normalized, (2, 0, 1))
    img_batch = np.expand_dims(img_chw, axis=0)
